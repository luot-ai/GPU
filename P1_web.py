...
def farthest_point_sample_single(xyz, npoint):
    """
    Input:
        xyz: pointcloud data, [N, 3] (for batch=1)
        npoint: number of samples
    Return:
        centroids: sampled pointcloud index, [npoint]
    """
    device = xyz.device
    N, C = xyz.shape  # For batch=1, we only have N points
    centroids = torch.zeros(npoint, dtype=torch.long).to(device)
    distance = torch.ones(N).to(device) * 1e10
    farthest = torch.randint(0, N, (1,), dtype=torch.long).to(device)  # Randomly select the first point
    for i in range(npoint):
        centroids[i] = farthest
        centroid = xyz[farthest, :].view(1, 3)  # Get the coordinates of the farthest point
        dist = torch.sum((xyz - centroid) ** 2, -1)  # Calculate squared distances
        mask = dist < distance
        distance[mask] = dist[mask]
        farthest = torch.max(distance, 0)[1]  # Select the next farthest point
