...
bool check(const float *A,
    ...
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            ...
            // }
            float sum = C1[i*n+j];
            if (std::fabs(sum - C[i * n + j]) / std::fabs(sum) > 1e-2f) {
...
__global__ void CBR_1024x128_kernel(int M,int batchSize,int N,int K,float* input, 
float* convWeights,float* output)
{   
    // param-set : variable
    int BM = 128;
    int BN = 128;
...
int main() {
    ...
    // warmup
    dim3 blockDim(32,32);
    dim3 gridDim((n + 32 - 1) / 32,(m + 32 - 1) / 32,1);//X:宽度 Y：高度

    //CBRWRAP_Kernel_np8tms<32, 32><<<gridDim, blockDim>>>(m,1,n,k,d_B,d_A,d_C1);
    CBR_1024x128_kernel<<<grid, 256>>>(m,1,n,k,d_B,d_A,d_C1);
    ...
    //     CBR_1024x128_kernel<<<grid, 256>>>(m,1,n,k,d_B,d_A,d_C);
    //     // CBRWRAP_Kernel_np8tms<32, 32><<<gridDim, blockDim>>>(m,1,n,k,d_B,d_A,d_C);
    //     // sgemm_128x128x8_kernel<<<grid, 256>>>(
    //     //     d_A, d_B, d_C, m, n, k, k * sizeof(float), n * sizeof(float) * 8);
    // }
    cudaEventRecord(end);
    ...
    cudaMemcpy(h_C1, d_C1, m * n * sizeof(float), cudaMemcpyDefault);

    bool chk = check(h_A, h_B, h_C,h_C1, m, n, k);
