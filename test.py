...
def get_batch_gemm_config():
    return {
        (1000, 128, 3, 3): (64, 64, 32, 300),  
...
def grouped_gemm_kernel(
    ...
    for batch in range(batchSize):
        ...
        if isCF == False:
            B_ptr += batch * K * N
        C_ptr = C + batch * M * N
        # a block process several tiles through all batches
        # SM blocks process a matrix through all batches
        while (tile_idx >= last_problem_end and tile_idx < last_problem_end + num_tiles):
            ...
            if BN == True:
                bnrm_vec = tl.load(bnrm + offs_bn_2D, mask= mask_bn)
                bnrv_vec = tl.load(bnrv + offs_bn_2D, mask=mask_bn)
                bnw_vec = tl.load(bnw + offs_bn_2D, mask=mask_bn)
                gamma_vec = bnw_vec / tl.sqrt(bnrv_vec + 1e-5)
                beta_vec = tl.load(bnb + offs_bn_2D, mask=mask_bn)

                bnrm_matrix = tl.broadcast_to(bnrm_vec, (BLOCK_SIZE_M, BLOCK_SIZE_N))
                gamma = tl.broadcast_to(gamma_vec, (BLOCK_SIZE_M, BLOCK_SIZE_N))
                beta = tl.broadcast_to(beta_vec, (BLOCK_SIZE_M, BLOCK_SIZE_N))
                accumulator = gamma * (accumulator - bnrm_matrix) + beta
            if RELU == True:
...
def bmm(A, B, batchSize, M, N, K):
    ...
    (blk_m, blk_n, blk_k, NUM_SM) = get_batch_gemm_config()[(batchSize, M, K, N)]
